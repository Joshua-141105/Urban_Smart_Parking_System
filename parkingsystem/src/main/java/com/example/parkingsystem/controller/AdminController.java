...
public class AdminController {
    ...
    // FIX: Added SYSTEM_ADMIN so sysadmin can also create offline bookings
    ...
    public ResponseEntity<?> createOfflineBooking(@RequestBody Map<String, Object> request, Principal principal) {
        try {
            ...
            // Verify the admin owns this lot — SYSTEM_ADMIN bypasses ownership check
            ParkingLot lot = parkingLotRepository.findById(lotId)
                    .orElseThrow(() -> new RuntimeException("Parking lot not found"));

            boolean isSystemAdmin = admin.getRole() == com.example.parkingsystem.entity.Role.SYSTEM_ADMIN;
            if (!isSystemAdmin && (lot.getOwner() == null || !lot.getOwner().getId().equals(admin.getId()))) {
    ...
    // FIX: Added @Transactional so lazy-loaded lot.getOwner() is accessible,
    //      and added SYSTEM_ADMIN role so sysadmin can view all lots
    @GetMapping("/my-lots")
    @PreAuthorize("hasRole('CITY_ADMIN') or hasRole('PARKING_MANAGER') or hasRole('SYSTEM_ADMIN')")
    ...
    public ResponseEntity<List<Map<String, Object>>> getMyParkingLots(Principal principal) {
        ...
        // FIX: SYSTEM_ADMIN sees all lots; others see only their own
        if (user.getRole() == com.example.parkingsystem.entity.Role.SYSTEM_ADMIN) {
            lots = parkingLotRepository.findAll();
        } else {
            lots = parkingLotRepository.findByOwnerId(user.getId());
        }
        ...
        List<Map<String, Object>> response = lots.stream().map(lot -> {
            ...
            // FIX: Use count query instead of loading all spaces to avoid N+1
            long occupied = parkingSpaceRepository.countByParkingLotIdAndIsOccupiedTrue(lot.getId());
            long available = lot.getTotalCapacity() - occupied;
            map.put("availableSpaces", available);
            map.put("occupancyPercentage",
                    lot.getTotalCapacity() > 0
                            ? Math.round((double) occupied / lot.getTotalCapacity() * 100.0)
                            : 0);
    ...
    @GetMapping("/bookings")
    @PreAuthorize("hasRole('PARKING_MANAGER') or hasRole('CITY_ADMIN') or hasRole('SYSTEM_ADMIN')")
    // FIX: @Transactional is essential here — the original had it but accessing
    //      lazy associations (booking.getUser(), booking.getParkingSpace(), etc.)
    //      outside a transaction caused the 500. Keeping readOnly=true for performance.
    @Transactional(readOnly = true)
    public ResponseEntity<?> getBookings(Principal principal) {
        ...
        // FIX: All association access happens inside the @Transactional method,
        //      so Hibernate session is still open — no LazyInitializationException
